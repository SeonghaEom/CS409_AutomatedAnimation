...
def heart2_effect (cap, frame, back_cap, back_frame, out, in_video, effect_path, who, i) :
    ...
    while(cap.isOpened()):
        ...
        for j in range(len(ani_start)):
            if start <= i < start+n :
                eff = cv2.imread( effect_path+'/heart_2/animation_heart_02-'+str(i-start).zfill(4)+'.jpg')
...
def heart2_effects (cap, frame, back_cap, back_frame, out, in_video, effect_path, i) :
    for j in range(in_video.hum_cnt):
        i, frame, back_frame = heart2_effect(cap,frame, back_cap,back_frame, out, in_video, effect_path, j, i)
    return i, frame, back_frame