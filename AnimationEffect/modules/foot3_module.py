...
def foot3_effect (cap, frame, back_cap, back_frame, out, in_video, effect_path, i) :
    ...
    while(cap.isOpened()):
        ...
        for j in range(len(ani_start)):
            if start <= i < start+n :
                eff = cv2.imread(effect_path+'/Foot3/foot3_back'+str(i-start).zfill(4)+'.jpg')
        ...
        for j in range(len(ani_start)):
            if start <= i < start+n :
                eff = cv2.imread(effect_path+'/Foot3/foot3'+str(i-start).zfill(4)+'.jpg')
                eff = cv2.resize(eff, dsize=(eff.shape[1]*standard_height//eff.shape[0], standard_height), interpolation=cv2.INTER_LINEAR)
