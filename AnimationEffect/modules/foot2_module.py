...
def foot2_effect (cap, frame, back_cap, back_frame, out, in_video, effect_path, i) :
    ...
    while(cap.isOpened()):
        ...
        for j in range(len(fr_humans)):
            ...
            if i == start:
                ...
                standard_height = int((anchors[13][1]-anchors[2][1])/3) #  knee - eye
        
        #draw
        for j in range(len(ani_start)):
            if start <= i < start+n :
                eff = cv2.imread(effect_path+'/Foot2/foot2'+str(i-start).zfill(4)+'.jpg')
