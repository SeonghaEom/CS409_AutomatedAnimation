...
def heart1_effect (cap, frame, back_cap, back_frame, out, in_video, effect_path, i) :
    ...
    while(cap.isOpened()):
        ...
        for j in range(len(ani_start)):
            if start <= i < start+n :
                eff = cv2.imread(effect_path+'/heart_1/animation_heart_01-'+str(i-start).zfill(4)+'.jpg')
